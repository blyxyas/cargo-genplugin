...
use syn::{
    Signature, __private::ToTokens, punctuated::Punctuated, token::Comma, FnArg,
};
...
",
    ...
    buf.write_all(
        format!(
            "// File autogenerated by cargo-plugin, you can use the `--fmt` flag to generate formatted files. Do not manually edit, it will be overwritten. Report issues @ github.com/blyxyas/cargo-plugin
use libloading;
			use	lazy_static;
			use std::error::Error;
        ...
        if let Some(sig) = parser::parse_to_fn_sig(&item) {
            ...
            buf.write_all(
                format!(
					...
					pub unsafe extern fn {}({}) -> Result<{}, Box<dyn Error>> {{ \
					\nlet func: libloading::Symbol<unsafe extern {}> = LIB.get(b\"{}\")?; \
					\nreturn Ok(func({})); \
					}}\n",
                    sig.ident,
